// src/app/pages/crm/dashboard/DashboardPage.tsx
...
const CRMDashboardPage: FC = () => {
  ...
  // 自動選擇 CRM 系統
  useEffect(() => {
    const crmSystem = systems.find(s => s.code?.toUpperCase() === 'CRM')
    if (crmSystem) {
      setSelectedSystemUuid(crmSystem.id)
    }
  ...
  const sectionCards = useMemo(() => {
    ...
    MENU_CONFIG.forEach(section => {
      ...
      section.items.forEach(item => {
        ...
        if (item.to === '/crm/dashboard' || item.to.includes('/:id/') || item.to.includes('/:uuid/')) {
          return
        }
    ...
    <Content>
      <div className='row g-5 g-xl-10 mb-5 mb-xl-10'>
          ...
          <div className='card card-flush'>
            <div className='card-header pt-7'>
              <div className='card-title'>
                <h2 className='fw-bold text-gray-800'>CRM 客戶管理系統</h2>
              </div>
            </div>
            <div className='card-body pt-6'>
              <p className='text-gray-600 fs-6 mb-0'>
                以下是 CRM 系統的可用功能模組，請選擇一個模組開始使用。
              </p>
            </div>
          </div>
        </div>
        ...
        {sectionCards.length === 0 ? (
          ...
          <div className='col-12'>
            <div 
              className='d-flex gap-5 overflow-x-auto pb-3 crm-dashboard-scroll'
              style={{
                scrollbarWidth: 'thin',
