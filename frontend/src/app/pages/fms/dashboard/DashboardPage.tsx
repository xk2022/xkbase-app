// src/app/pages/fms/dashboard/DashboardPage.tsx
...
const FMSDashboardPage: FC = () => {
  ...
  // 自動選擇 FMS 系統
  useEffect(() => {
    const fmsSystem = systems.find(s => s.code?.toUpperCase() === 'FMS')
    if (fmsSystem) {
      setSelectedSystemUuid(fmsSystem.id)
    }
  ...
  const sectionCards = useMemo(() => {
    ...
    MENU_CONFIG.forEach(section => {
      ...
      section.items.forEach(item => {
        ...
        if (item.to === '/fms/dashboard' || item.to.includes('/:id/') || item.to.includes('/:uuid/')) {
          return
        }
    ...
    <Content>
      <div className='row g-5 g-xl-10 mb-5 mb-xl-10'>
          ...
          <div className='card card-flush'>
            <div className='card-header pt-7'>
              <div className='card-title'>
                <h2 className='fw-bold text-gray-800'>FMS 車輛運輸系統</h2>
              </div>
            </div>
            <div className='card-body pt-6'>
              <p className='text-gray-600 fs-6 mb-0'>
                以下是 FMS 系統的可用功能模組，請選擇一個模組開始使用。
              </p>
            </div>
          </div>
        </div>
        ...
        {sectionCards.length === 0 ? (
          ...
          <div className='col-12'>
            <div 
              className='d-flex gap-5 overflow-x-auto pb-3 fms-dashboard-scroll'
              style={{
                scrollbarWidth: 'thin',
            ...
            >
              <style>{`
                .fms-dashboard-scroll::-webkit-scrollbar {
                  height: 8px;
                }
                .fms-dashboard-scroll::-webkit-scrollbar-track {
                  background: transparent;
                }
                .fms-dashboard-scroll::-webkit-scrollbar-thumb {
                  background-color: #cbd5e0;
                  border-radius: 4px;
                }
                .fms-dashboard-scroll::-webkit-scrollbar-thumb:hover {
