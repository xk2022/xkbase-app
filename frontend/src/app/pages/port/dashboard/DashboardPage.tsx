// src/app/pages/port/dashboard/DashboardPage.tsx
...
const PortDashboardPage: FC = () => {
  ...
  // 自動選擇 PORT 系統
  useEffect(() => {
    const portSystem = systems.find(s => s.code?.toUpperCase() === 'PORT')
    if (portSystem) {
      setSelectedSystemUuid(portSystem.id)
    }
  ...
  const sectionCards = useMemo(() => {
    ...
    MENU_CONFIG.forEach(section => {
      // 只處理 PORT 系統的區塊
      if (section.requiredSystem !== 'PORT') {
        return
      }
      ...
      section.items.forEach(item => {
        ...
        if (item.to === '/port/dashboard' || item.to.includes('/:id/') || item.to.includes('/:uuid/')) {
          return
        }
    ...
    <Content>
      <div className='row g-5 g-xl-10 mb-5 mb-xl-10'>
          ...
          <div className='card card-flush'>
            <div className='card-header pt-7'>
              <div className='card-title'>
                <h2 className='fw-bold text-gray-800'>PORT 港口整合系統</h2>
              </div>
            </div>
            <div className='card-body pt-6'>
              <p className='text-gray-600 fs-6 mb-0'>
                以下是 PORT 系統的可用功能模組，請選擇一個模組開始使用。
              </p>
            </div>
          </div>
        </div>
        ...
        {sectionCards.length === 0 ? (
          ...
          <div className='col-12'>
            <div 
              className='d-flex gap-5 overflow-x-auto pb-3 port-dashboard-scroll'
              style={{
                scrollbarWidth: 'thin',
            ...
            >
              <style>{`
                .port-dashboard-scroll::-webkit-scrollbar {
                  height: 8px;
                }
                .port-dashboard-scroll::-webkit-scrollbar-track {
                  background: transparent;
                }
                .port-dashboard-scroll::-webkit-scrollbar-thumb {
                  background-color: #cbd5e0;
                  border-radius: 4px;
                }
                .port-dashboard-scroll::-webkit-scrollbar-thumb:hover {
